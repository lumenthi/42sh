...
/*   Updated: 2018/08/25 10:55:46 by lumenthi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		history_add(char *cmd)
{
	...
	if (g_history->no_keep == 0)
	{
		size = tab_size(g_history->line);
...
static int	size_error(void)
{
	...
	ft_putstr_fd("21sh", 2);
	ft_putstr_fd(BLANK, 2);
	ft_putendl_fd(": invalid HISTSIZE value, setting it to default (500)", 2);
	g_history->SIZE = 500;
	return (0);
}
...
int			history_init(void)
{
	...
	g_history->nb_lines = tab_size(g_history->line);
	g_history->start  = tab_size(g_history->line);
	return (1);
}
